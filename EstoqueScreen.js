...
export default class EstoqueScreen extends React.Component {
  ...
  handleConfirmar = () => {
    ...
    this.socket.emit('atualizar_estoque', { itensAlterados,username:user.username,token:user.token });
...
const {user} =  this.context
if (AdicionarItem){
this.socket.emit('EditingEstoque', {tipo:titleEnv,item:AdicionarItem,novoNome:AdicionarNovoNome,quantidade:AdicionarQuantidade, estoqueIdeal:AdicionarEstoqueIdeal,estoque:'estoque',username:user.username,token:user.token});
