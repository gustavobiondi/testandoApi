import React from 'react';
  ...
  export default class EstoqueScreen extends React.Component {
    ...
    handleConfirmar = () => {
      ...
      this.setState({ showEditar: false,estoque:''});
    };

    searchEstoque = (estoque) =>{
      const data_filtrado = this.state.dataGeral.filter(item => item.item.startsWith(estoque))
      this.setState({estoque,data:data_filtrado})
    }
