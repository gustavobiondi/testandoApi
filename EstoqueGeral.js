...
export default class EstoqueGeral extends React.Component {
  ...
  handleConfirmar = () => {
    ...
    this.socket.emit('atualizar_estoque_geral', { itensAlterados,username:user.username,token:user.token });
  ...
  Enviar=()=>{
    ...
    if(AdicionarItem){
    this.socket.emit("EditingEstoque",{item:AdicionarItem,novoNome:AdicionarNovoNome,quantidade:AdicionarQuantidade,estoqueIdeal:AdicionarEstoqueIdeal,tipo:titleEnv,estoque:'estoque_geral',username:user.username,token:user.token})
